function certify_genericity_b (L,b)
    #TODO
    return b
end


function intersection(TropV1::TropicalVarietySupertype{minOrMax,true}, TropV2::TropicalVarietySupertype{minOrMax,true})
    ...
    # todo: check that perturbation is actually generi
    for sigma1 in maximal_polyhedra(TropV1)
        for sigma2 in maximal_polyhedra(TropV2)
            if dim(sigma1+sigma2)==n
            ...
            if dim(sigma1+sigma2)==n  && dim(sigma12)=0
                sigma12 = intersect(sigma1, sigma2)
